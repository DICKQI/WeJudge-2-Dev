...
class JWSpider(object):
    ...
    def login_validate(self, stucode, passwd):
        ...
        req = urllib.request.Request("http://es.bnuz.edu.cn/default2.aspx", headers=header,
        ...
        try:
            ...
            self.is_login = False
            if u"密码不正确" in ctx:
                raise WeJudgeError(1900)
            elif u"5分钟" in ctx:
                raise WeJudgeError(1901)
            else:
                raise WeJudgeError(1902)
