...
(function (THREE, $, TweenLite, Power3, TimelineMax) {
	"use strict";
	...
	function init() {
		...
		// Render To DOM
		document.getElementById('bg_container').appendChild(renderer.domElement);
		...
		// Mesh creation
		var geometry = new THREE.PlaneGeometry(400, 400, 30, 30);
		...
		// Create stars
		// farthestStars = createStars(1200, 420, "#ffffff");
		// farStars = createStars(1200, 370, "#ffffff");
		// nearStars = createStars(1200, 290, "#ffffff");
	...
	function render() {
		...
		if (intersects.length > 0) {
			(function () {
				...
				// 小三角的颜色，默认：#006ea0
				intersects[0].face.color.setStyle("#666666");
				plane.geometry.colorsNeedUpdate = true;
			})();
		}
	...
	var xMark = $('#close-mark');

	$('.shift-camera-button').click(function () {
		var introTimeline = new TimelineMax();

		introTimeline.add([TweenLite.fromTo(introContainer, 0.5, { opacity: 1 }, { display: "none", opacity: 0, ease: Power3.easeIn }), TweenLite.to(camera.rotation, 2, { x: Math.PI / 2, ease: Power3.easeInOut }), TweenLite.to(camera.position, 1.5, { z: 20, ease: Power3.easeInOut }), TweenLite.to(camera.position, 2, { y: 120, ease: Power3.easeInOut }), TweenLite.to(plane.scale, 2, { x: 2, ease: Power3.easeInOut })]);

		introTimeline.add([TweenLite.to(xMark, 2, {opacity: 1, display: "block",  ease: Power3.easeInOut }), TweenLite.to(skyContainer, 2, { opacity: 1, display: "block",  ease: Power3.easeInOut })]);
	});

	xMark.click(function () {
		var outroTimeline = new TimelineMax();

		outroTimeline.add([TweenLite.to(xMark, 0.5, { opacity: 0, display: "none",  ease: Power3.easeInOut }), TweenLite.to(skyContainer, 0.5, { opacity: 0, display: "none",  ease: Power3.easeInOut }), TweenLite.to(camera.rotation, 3, { x: 0, ease: Power3.easeInOut }), TweenLite.to(camera.position, 3, { z: 50, ease: Power3.easeInOut }), TweenLite.to(camera.position, 2.5, { y: 0, ease: Power3.easeInOut }), TweenLite.to(plane.scale, 3, { x: 1, ease: Power3.easeInOut })]);

		outroTimeline.add([TweenLite.to(introContainer, 0.5, { opacity: 1, display: "block",  ease: Power3.easeIn })]);
	});



	$.extend({
        WejudgeIndexAnimate: function () {
...
})(THREE, jQuery, TweenLite, Power3, TimelineMax);